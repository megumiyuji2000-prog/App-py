...
def proses_video(file):
    try:
        ...
        frame_step = max(1, total_frames // 5)
...
with col1:
    st.write("✨") # Ganti pake st.image("fnl_logo.png", width=50) kalo udah upload
with col2:
        ...
        if video_file.size > 40 * 1024 * 1024:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Nonton videonya dulu bro... 🎬"):
                    frames = proses_video(video_file)
                if frames:
                    st.info(f"Dapet {len(frames)} cuplikan dari video. Lagi dianalisis...")
                    placeholder = st.empty(); full_response = ""
                    stream = chat_vision(frames, user_text)
                    if stream:
                        for chunk in stream:
                            if chunk.choices[0].delta.content:
                                full_response += chunk.choices[0].delta.content
