...
if sisa<=3 and sisa>0:st.toast(f"waduh waktu ngobrol sisa {sisa} Kali lagi, nih siap-siap ya",icon="⚠️")
...
def generate_gambar(p):
 toast("maaf jika gambar kurang memuaskan🙏","🎨");st.session_state.last_generated_prompt=p
 ...
 try:r=requests.get(url,timeout=60);return(Image.open(io.BytesIO(r.content)).convert("RGB"),None)if r.status_code==200 else(None,"Server penuh")
...
def remix_gambar_hasil_generate(pr):
 ...
 toast("maaf jika gambar kurang memuaskan🙏","✨");fp=f"{st.session_state.last_generated_prompt}, {pr}";st.session_state.last_generated_prompt=fp
 ...
 try:r=requests.get(url,timeout=60);return(Image.open(io.BytesIO(r.content)).convert("RGB"),None)if r.status_code==200 else(None,"Gagal remix")
...
def kirim_ke_ai(prompt,image=None):
 ...
 for try_model in models:
  try:
   if time.time()-start_time>55:toast("aduh seperti nya sistemnya lagi capek,coba lagi nanti,atau refresh halaman ya☺️🙏","😅");return[("text","aduh seperti nya sistemnya lagi capek,coba lagi nanti,atau refresh halaman ya☺️🙏","ngobrol")]
   if try_model=="gemini":
    toast("Pake Gemini...","✨")
    content=[full_p]
   ...
   elif"timeout"in err.lower()or"time"in err.lower():toast("aduh seperti nya sistemnya lagi capek,coba lagi nanti,atau refresh halaman ya☺️🙏","😅");return[("text","aduh seperti nya sistemnya lagi capek,coba lagi nanti,atau refresh halaman ya☺️🙏","ngobrol")]
...
function injectButtons(){{
 const chatInput=document.querySelector('[data-testid="stChatInput"] > div');
 if(!chatInput)return;
 if(!document.getElementById('custom-plus-btn')){{
  const plusBtn=document.createElement('button');
  plusBtn.innerHTML='+';
  plusBtn.id='custom-plus-btn';
  plusBtn.style.cssText='position:absolute;left:12px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;background:transparent;border:none;color:{T['icon']};font-size:1.6rem;font-weight:300;cursor:pointer;z-index:10003;display:flex;align-items:center;justify-content:center;line-height:1;padding:0;';
  plusBtn.onmouseover=()=>plusBtn.style.background='{T['user_bg']}';
  plusBtn.onmouseout=()=>plusBtn.style.background='transparent';
  plusBtn.onclick=()=>document.querySelector('[data-testid="stFileUploader"] input').click();
  chatInput.appendChild(plusBtn);
 }}
 if(!document.getElementById('custom-mic-btn')){{
  const micBtn=document.createElement('button');
  micBtn.innerHTML='🎤';
  micBtn.id='custom-mic-btn';
  micBtn.style.cssText='position:absolute;left:44px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;background:transparent;border:none;color:{T['icon']};font-size:1.1rem;cursor:pointer;z-index:10003;display:flex;align-items:center;justify-content:center;padding:0;';
  micBtn.onmouseover=()=>micBtn.style.background='{T['user_bg']}';
  micBtn.onmouseout=()=>micBtn.style.background='transparent';
  micBtn.onclick=()=>document.querySelector('[data-testid="stAudioInput"] button').click();
  chatInput.appendChild(micBtn);
 }}
}}
const observer=new MutationObserver(injectButtons);
observer.observe(document.body,{{childList:true,subtree:true}});
setInterval(injectButtons,300);
...
if upload_file:
 ...
 for tipe,konten,*rest in hasil:
  tingkat=rest[0]if rest else"ngobrol";model=rest[1]if len(rest)>1 else st.ses
